...
<sub>Animated: the login, then the read path (red) and the privileged path (green). Each namespace's JWT-SVID drops from SPIRE as the request reaches it, and each hop exchanges its token at Curity. Click for the static diagram; regenerate with `python3 scripts/render-architecture-svg.py`.</sub>
