...
<sub>Animated: SVIDs from SPIRE, the login, then the read path (red) and the privileged path (green), each hop exchanging its token at Curity. Click for the static diagram; regenerate with `python3 scripts/render-architecture-svg.py`.</sub>
