...
describe('ExchangeCache', () => {
  it('returns a stored token for the same caller token + audience', () => {
    ...
    c.set('caller.jwt', 'mcp-observability', token);
    expect(c.get('caller.jwt', 'mcp-observability')?.access_token).toBe('narrowed');
  });

  it('keys on the WHOLE caller token, so a re-issued token (new jti) is a miss', () => {
    const c = new ExchangeCache({ ttlSeconds: 60, maxEntries: 10 });
    c.set('caller.jwt.A', 'mcp-observability', token);
    expect(c.get('caller.jwt.B', 'mcp-observability')).toBeUndefined();
  });
  ...
  it('never stores the raw caller token as the key', () => {
    const c = new ExchangeCache({ ttlSeconds: 60, maxEntries: 10 });
    c.set('super-secret-caller-jwt', 'mcp-observability', token);
  ...
  it('expires after the configured TTL', () => {
    ...
    expect(c.get('caller.jwt', 'mcp-observability')).toBeDefined();
    vi.advanceTimersByTime(1_001);
    expect(c.get('caller.jwt', 'mcp-observability')).toBeUndefined();
  });
  ...
  it('bounds the TTL by the issued token lifetime minus the skew, whichever is shorter', () => {
    ...
    c.set('caller.jwt', 'mcp-observability', { ...token, expires_in: 90 });
    vi.advanceTimersByTime(59_000);
    expect(c.get('caller.jwt', 'mcp-observability')).toBeDefined();
    vi.advanceTimersByTime(1_001);
    expect(c.get('caller.jwt', 'mcp-observability')).toBeUndefined();
  ...
  it('does not store a token whose lifetime is within the skew', () => {
    ...
    c.set('caller.jwt', 'mcp-observability', { ...token, expires_in: 30 });
    expect(c.get('caller.jwt', 'mcp-observability')).toBeUndefined();
  });
  ...
  it('reports the REMAINING lifetime on a hit, not the original expires_in', () => {
    vi.useFakeTimers();
    const c = new ExchangeCache({ ttlSeconds: 60, maxEntries: 10 });
    c.set('caller.jwt', 'mcp-observability', { ...token, expires_in: 600 });
    vi.advanceTimersByTime(20_000);
    expect(c.get('caller.jwt', 'mcp-observability')?.expires_in).toBe(580);
  });
