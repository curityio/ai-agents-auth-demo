...
// Stub the inspect-api hop: this exercises the MCP transport + auth plumbing, not
// the backend call. `obtainInspectApiToken` echoes back the subject token it was
// handed so the test can assert the per-request bearer reached the tool.
const seenSubjectTokens: string[] = [];
vi.mock('../src/inspect-api-client.js', () => ({
  obtainInspectApiToken: async ({ subjectToken }: { subjectToken: string }) => {
