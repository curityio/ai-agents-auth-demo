...
export function buildLastTokenHandlers(cfg: Config): {
  ...
  return {
    ...
    handler(req: Request, res: Response) {
      ...
      // Inbound hop: the aud=mcp-ops token the agentgateway minted for us (via the
      // exchange-shim). Reveals the gateway → mcp leg the caller can't see (that
      // token is minted inside the gateway pod, not by the agent).
      const authz = req.header('authorization') ?? '';
