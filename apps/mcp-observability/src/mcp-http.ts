 ...
 * `authMiddleware`, which publishes the validated bearer as `req.auth`.
 *
...
export function buildMcpRequestHandler(cfg: Config): RequestHandler {
  // One factory, called once per HTTP request. Per-request construction is what
  // makes the per-caller `subjectToken` safe: nothing outlives the exchange it
  // was minted for.
  //
  ...
  const handler = createMcpHandler(
    (ctx) => {
      // authMiddleware already validated this bearer and published it as
      // AuthInfo; `toNodeHandler` forwards `req.auth` here untouched (the SDK
      // never reads credentials out of headers itself).
      const subjectToken = ctx.authInfo?.token ?? '';
      const subjectSub = String(ctx.authInfo?.extra?.sub ?? 'unknown');
      return buildMcpServer(cfg, { subjectToken, subjectSub });
    },
    {
      legacy: 'reject',
      onerror: (e) => console.error('[mcp-observability] handler error', e),
    },
  );
  const nodeHandler = toNodeHandler(handler, {
    onerror: (e) => console.error('[mcp-observability] adapter error', e),
  });
