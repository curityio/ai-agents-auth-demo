...
| BFF `GET /api/spiffe-identities?flow=read\|privileged` | session | the SVIDs of exactly the workloads in that flow, in chain order (`web → agent-copilot → [agent-specialist →] agentgateway → mcp-observability [→ mcp-ops]`) |
