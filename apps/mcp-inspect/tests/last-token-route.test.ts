/**
 * Unit tests for buildChain — the rows mcp-inspect's /last-token contributes to the
 * OBO-chain view.
 *
 * Both rows must come from the last REAL tool call. In particular the
 * "agentgateway → mcp-inspect" row must NOT be decoded from the /last-token request's
...
const INBOUND = jwt({ sub: 'alice', aud: 'mcp-inspect', jti: 'flow-inbound' });
const EXCHANGED = jwt({ sub: 'alice', aud: 'inspect-api', jti: 'flow-exchanged' });

describe('buildChain', () => {
  it('builds BOTH rows from the last tool call: inbound subject token, then the inspect-api token', () => {
    const chain = buildChain({ subjectToken: INBOUND, accessToken: EXCHANGED, at: 1 }, false);
    expect(chain.map((h) => h.hop)).toEqual(['agentgateway → mcp-inspect', 'mcp-inspect → inspect-api']);
