...
// Snapshot of the most recent mcp-inspect → inspect-api exchange, for the
// /last-token OBO-chain visualization. Single global slot (demo only).
// `subjectToken` is the INBOUND aud=mcp-inspect token this call ran with — kept so
// /last-token can show the real agentgateway → mcp-inspect leg. It must not decode
...
/**
 * 3rd RFC 8693 hop for the read path: subject = inbound mcp-inspect-bound
 * token (act={copilot}); actor = mcp-inspect's SVID. Curity nests to
 * act={mcp-inspect,copilot}. Audience-only narrowing; scope stays inspect:read.
 */
export async function obtainInspectApiToken(opts: {
  cfg: Config;
  subjectToken: string;
}): Promise<string> {
  ...
  const result = await exchangeToken({
    ...
    subjectToken,
    actorToken: svid.jwt,
    audience: cfg.inspectApiAudience,
    scope: cfg.inspectApiScope,
  });
...
async function getJson<T>(url: string, bearer: string): Promise<T> {
  ...
  if (!res.ok) {
    ...
    throw new Error(`inspect-api responded ${res.status}${detail ? ': ' + detail : ''}`);
  }
...
export async function callListPods(opts: {
  ...
  const url = `${opts.cfg.inspectApiBaseUrl}/pods?namespace=${encodeURIComponent(opts.namespace)}`;
  oboLog({
    service: 'mcp-inspect',
    kind: 'CALL',
    headline: '→ inspect-api GET /pods',
...
export async function callGetPodLogs(opts: {
  ...
  const url =
    `${opts.cfg.inspectApiBaseUrl}/pods/${encodeURIComponent(opts.podName)}/logs` +
    `?namespace=${encodeURIComponent(opts.namespace)}&tailLines=${opts.tailLines}`;
  oboLog({
    service: 'mcp-inspect',
    kind: 'CALL',
    headline: '→ inspect-api GET /pods/:name/logs',
    fields: {
      pod: opts.podName,
...
export async function callGetDeployment(opts: {
  ...
  const url = `${opts.cfg.inspectApiBaseUrl}/deployments/${encodeURIComponent(opts.name)}?namespace=${encodeURIComponent(opts.namespace)}`;
  oboLog({
    service: 'mcp-inspect',
    kind: 'CALL',
    headline: '→ inspect-api GET /deployments/:name',
