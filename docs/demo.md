...
- An **Azure OpenAI** (AI Foundry) endpoint + API key for the agent LLM —
  `make demo` prompts for these up front.
...
to copy it in if absent) and prompts for the Azure OpenAI endpoint + API key
(saved to a gitignored `.demo.env`). Then it runs **unattended**:
...
- **seed-secrets** installs the license (`curity-license` from `./license.json`),
  ...
  which is the fixed demo value `Password1`), the Azure LLM secret for **both**
  agents (`agent-copilot-llm` + `agent-specialist-llm`, from `.demo.env`), the
